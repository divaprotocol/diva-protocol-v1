...
// [cut, loupe, loupe, loupe, loupe, erc165, transferOwnership, getOwner]
...
describe("Cache bug test", async () => {
  ...
  before(async function () {
    ...
    const diamondAddress = (await deployMain())[0];
  ...
  it("should not exhibit the cache bug", async () => {
    ...
    const selectors = await diamondLoupeFacet.facetFunctionSelectors(
      test1Facet.address
    );
